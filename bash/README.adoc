...
* TODO Opening brace after the name, or on a new line?
...
When writing `$foo` on its own instead of `"$foo"`, you’re asking the Bash interpreter to split the contents of the `foo` variable into multiple words in every place where a character of `IFS` (by default this includes spaces, tabs and newlines) is found:
...
See https://stackoverflow.com/a/71600549/9089759[this StackOverflow answer].
...
It is also generally a good idea to check that the mock was called with the *expected arguments*:
